...
primrec env :: "nat => envType" where
  "env N (Ident n) = (if Ident n = Ident ''Sta.Dir.Pending'' then boolType else if Ident n = Ident ''Sta.Dir.Local'' then boolType else if Ident n = Ident ''Sta.Dir.Dirty'' then boolType else if Ident n = Ident ''Sta.Dir.HeadVld'' then boolType else if Ident n = Ident ''Sta.Dir.ShrVld'' then boolType else if Ident n = Ident ''Sta.Dir.HeadPtr'' then indexType else if Ident n = Ident ''Sta.WbMsg.Cmd'' then enumType else if Ident n = Ident ''Sta.WbMsg.Proc'' then indexType else if Ident n = Ident ''Sta.ShWbMsg.Cmd'' then enumType else if Ident n = Ident ''Sta.ShWbMsg.Proc'' then indexType else if Ident n = Ident ''Sta.NakcMsg.Cmd'' then enumType else if Ident n = Ident ''Sta.HomeProc.ProcCmd'' then enumType else if Ident n = Ident ''Sta.HomeProc.InvMarked'' then boolType else if Ident n = Ident ''Sta.HomeProc.CacheState'' then enumType else if Ident n = Ident ''Sta.HomeUniMsg.Cmd'' then enumType else if Ident n = Ident ''Sta.HomeUniMsg.HomeProc'' then boolType else if Ident n = Ident ''Sta.HomeUniMsg.Proc'' then indexType else if Ident n = Ident ''Sta.HomePendReqSrc'' then boolType else if Ident n = Ident ''Sta.Collecting'' then boolType else if Ident n = Ident ''Sta.FwdCmd'' then enumType else if Ident n = Ident ''Sta.PendReqSrc'' then indexType else anyType)" |
...
lemma symPreds20 [intro]: 
  ...
  unfolding initSpec20_def
  apply (rule symPredSetExistForall)
...
lemma symPI_Local_GetX_PutX: 
  ...
  apply (auto intro!: equivStatementParallel equivStatementIteStm equivStatementPermute)
