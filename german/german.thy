...
primrec env :: "nat => envType" where
  "env N (Ident n) = (if Ident n = Ident ''ExGntd'' then boolType else if Ident n = Ident ''CurCmd'' then enumType else if Ident n = Ident ''CurPtr'' then indexType else anyType)" |
  "env N (Para n i) = (if Para n i = Para ''Chan1.Cmd'' i \<and> (i \<le> N) then enumType else if Para n i = Para ''Chan2.Cmd'' i \<and> (i \<le> N) then enumType else if Para n i = Para ''Chan3.Cmd'' i \<and> (i \<le> N) then enumType else if Para n i = Para ''Cache.State'' i \<and> (i \<le> N) then enumType else if Para n i = Para ''InvSet'' i \<and> (i \<le> N) then boolType else if Para n i = Para ''ShrSet'' i \<and> (i \<le> N) then boolType else anyType)" |
  "env N dontCareVar = anyType"

lemma env_simp: 
  "i \<le> N \<Longrightarrow> env N (Para ''Chan1.Cmd'' i) = enumType"
  "i \<le> N \<Longrightarrow> env N (Para ''Chan2.Cmd'' i) = enumType"
  "i \<le> N \<Longrightarrow> env N (Para ''Chan3.Cmd'' i) = enumType"
  "i \<le> N \<Longrightarrow> env N (Para ''Cache.State'' i) = enumType"
  "i \<le> N \<Longrightarrow> env N (Para ''InvSet'' i) = boolType"
  "i \<le> N \<Longrightarrow> env N (Para ''ShrSet'' i) = boolType"
...
lemma deriveFormInitSpec1 [intro]: 
  "f \<in> {initSpec1 N} \<Longrightarrow> deriveForm (env N) f"
  apply auto
  done
...
lemma deriveFormInitSpec2 [intro]: 
  "f \<in> {initSpec2 N} \<Longrightarrow> deriveForm (env N) f"
  apply auto
  done
...
lemma deriveFormInitSpec3 [intro]: 
  "f \<in> {initSpec3 N} \<Longrightarrow> deriveForm (env N) f"
  apply auto
  done
...
lemma deriveFormInitSpec4 [intro]: 
  "f \<in> {initSpec4 N} \<Longrightarrow> deriveForm (env N) f"
  apply auto
  done
...
lemma deriveFormInitSpec7 [intro]: 
  "f \<in> {initSpec7} \<Longrightarrow> deriveForm (env N) f"
  apply auto
  done
...
lemma deriveFormInitSpec8 [intro]: 
  "f \<in> {initSpec8 N} \<Longrightarrow> deriveForm (env N) f"
  apply auto
  done
...
lemma deriveFormAllInitSpec: 
  ...
  using deriveFormInitSpec0 deriveFormInitSpec1 deriveFormInitSpec2 deriveFormInitSpec3 deriveFormInitSpec4 deriveFormInitSpec5 deriveFormInitSpec6 deriveFormInitSpec7 deriveFormInitSpec8
  apply (auto simp del: initSpec0_def initSpec1_def initSpec2_def initSpec3_def initSpec4_def initSpec5_def initSpec6_def initSpec7_def initSpec8_def)
  done
...
lemma symRecvGntE: 
  ...
  "i \<le> N \<Longrightarrow> wellFormedRule (env N) N (RecvGntE i)"
  unfolding RecvGntE_def
...
lemma symRecvGntS: 
  ...
  "i \<le> N \<Longrightarrow> wellFormedRule (env N) N (RecvGntS i)"
  unfolding RecvGntS_def
  apply (auto intro!: symParamRuleI2 symParamRuleI symParamFormAnd symParamFormForall symParamFormForallExcl symParamFormImply symParamStatementParallel symParamStatementForall symParamStatementForallExcl symParamStatementIte)
  unfolding symParamForm_def symParamStatement_def symParamForm2_def symParamStatement2_def mutualDiffVars_def equivForm_def
  apply auto
  done
...
lemma symSendGntE: 
  ...
  "i \<le> N \<Longrightarrow> wellFormedRule (env N) N (SendGntE N i)"
  unfolding SendGntE_def
  apply (auto intro!: symParamRuleI2 symParamRuleI symParamFormAnd symParamFormForall symParamFormForallExcl symParamFormImply symParamStatementParallel symParamStatementForall symParamStatementForallExcl symParamStatementIte)
  unfolding symParamForm_def symParamStatement_def symParamForm2_def symParamStatement2_def mutualDiffVars_def equivForm_def
  apply auto
  done
...
lemma symSendGntS: 
  ...
  "i \<le> N \<Longrightarrow> wellFormedRule (env N) N (SendGntS i)"
  unfolding SendGntS_def
  apply (auto intro!: symParamRuleI2 symParamRuleI symParamFormAnd symParamFormForall symParamFormForallExcl symParamFormImply symParamStatementParallel symParamStatementForall symParamStatementForallExcl symParamStatementIte)
  unfolding symParamForm_def symParamStatement_def symParamForm2_def symParamStatement2_def mutualDiffVars_def equivForm_def
  apply auto
  done
...
lemma symRecvInvAck1: 
  ...
  "i \<le> N \<Longrightarrow> wellFormedRule (env N) N (RecvInvAck1 i)"
  unfolding RecvInvAck1_def
  apply (auto intro!: symParamRuleI2 symParamRuleI symParamFormAnd symParamFormForall symParamFormForallExcl symParamFormImply symParamStatementParallel symParamStatementForall symParamStatementForallExcl symParamStatementIte)
  unfolding symParamForm_def symParamStatement_def symParamForm2_def symParamStatement2_def mutualDiffVars_def equivForm_def
  apply auto
  done
...
lemma symRecvInvAck2: 
  ...
  "i \<le> N \<Longrightarrow> wellFormedRule (env N) N (RecvInvAck2 i)"
  unfolding RecvInvAck2_def
  apply (auto intro!: symParamRuleI2 symParamRuleI symParamFormAnd symParamFormForall symParamFormForallExcl symParamFormImply symParamStatementParallel symParamStatementForall symParamStatementForallExcl symParamStatementIte)
  unfolding symParamForm_def symParamStatement_def symParamForm2_def symParamStatement2_def mutualDiffVars_def equivForm_def
  apply auto
  done
...
lemma symSendInvAck: 
  ...
  "i \<le> N \<Longrightarrow> wellFormedRule (env N) N (SendInvAck i)"
  unfolding SendInvAck_def
  apply (auto intro!: symParamRuleI2 symParamRuleI symParamFormAnd symParamFormForall symParamFormForallExcl symParamFormImply symParamStatementParallel symParamStatementForall symParamStatementForallExcl symParamStatementIte)
  unfolding symParamForm_def symParamStatement_def symParamForm2_def symParamStatement2_def mutualDiffVars_def equivForm_def
  apply auto
  done
...
lemma symSendInv: 
  ...
  "i \<le> N \<Longrightarrow> wellFormedRule (env N) N (SendInv i)"
  unfolding SendInv_def
  apply (auto intro!: symParamRuleI2 symParamRuleI symParamFormAnd symParamFormForall symParamFormForallExcl symParamFormImply symParamStatementParallel symParamStatementForall symParamStatementForallExcl symParamStatementIte)
  unfolding symParamForm_def symParamStatement_def symParamForm2_def symParamStatement2_def mutualDiffVars_def equivForm_def
  apply auto
  done
...
lemma symRecvReqE: 
  ...
  "i \<le> N \<Longrightarrow> wellFormedRule (env N) N (RecvReqE N i)"
  unfolding RecvReqE_def
  apply (auto intro!: symParamRuleI2 symParamRuleI symParamFormAnd symParamFormForall symParamFormForallExcl symParamFormImply symParamStatementParallel symParamStatementForall symParamStatementForallExcl symParamStatementIte)
  unfolding symParamForm_def symParamStatement_def symParamForm2_def symParamStatement2_def mutualDiffVars_def equivForm_def
  apply auto
  done
...
lemma symRecvReqS: 
  ...
  "i \<le> N \<Longrightarrow> wellFormedRule (env N) N (RecvReqS N i)"
  unfolding RecvReqS_def
  apply (auto intro!: symParamRuleI2 symParamRuleI symParamFormAnd symParamFormForall symParamFormForallExcl symParamFormImply symParamStatementParallel symParamStatementForall symParamStatementForallExcl symParamStatementIte)
  unfolding symParamForm_def symParamStatement_def symParamForm2_def symParamStatement2_def mutualDiffVars_def equivForm_def
  apply auto
  done
...
lemma symSendReqE: 
  ...
  "i \<le> N \<Longrightarrow> wellFormedRule (env N) N (SendReqE i)"
  unfolding SendReqE_def
  apply (auto intro!: symParamRuleI2 symParamRuleI symParamFormAnd symParamFormForall symParamFormForallExcl symParamFormImply symParamStatementParallel symParamStatementForall symParamStatementForallExcl symParamStatementIte)
  unfolding symParamForm_def symParamStatement_def symParamForm2_def symParamStatement2_def mutualDiffVars_def equivForm_def
  apply auto
  done
...
lemma symSendReqS: 
  ...
  "i \<le> N \<Longrightarrow> wellFormedRule (env N) N (SendReqS i)"
  unfolding SendReqS_def
  apply (auto intro!: symParamRuleI2 symParamRuleI symParamFormAnd symParamFormForall symParamFormForallExcl symParamFormImply symParamStatementParallel symParamStatementForall symParamStatementForallExcl symParamStatementIte)
  unfolding symParamForm_def symParamStatement_def symParamForm2_def symParamStatement2_def mutualDiffVars_def equivForm_def
  apply auto
  done
...
lemma deriveAll: 
  "r \<in> RecvGntEs N \<Longrightarrow> deriveRule (env N) r"
  "r \<in> RecvGntSs N \<Longrightarrow> deriveRule (env N) r"
  "r \<in> SendGntEs N \<Longrightarrow> deriveRule (env N) r"
  "r \<in> SendGntSs N \<Longrightarrow> deriveRule (env N) r"
  "r \<in> RecvInvAck1s N \<Longrightarrow> deriveRule (env N) r"
  "r \<in> RecvInvAck2s N \<Longrightarrow> deriveRule (env N) r"
  "r \<in> SendInvAcks N \<Longrightarrow> deriveRule (env N) r"
  "r \<in> SendInvs N \<Longrightarrow> deriveRule (env N) r"
  "r \<in> RecvReqEs N \<Longrightarrow> deriveRule (env N) r"
  "r \<in> RecvReqSs N \<Longrightarrow> deriveRule (env N) r"
  "r \<in> SendReqEs N \<Longrightarrow> deriveRule (env N) r"
  "r \<in> SendReqSs N \<Longrightarrow> deriveRule (env N) r"
  "r \<in> ABS_SendGntEs N \<Longrightarrow> deriveRule (env N) r"
  "r \<in> ABS_SendGntSs N \<Longrightarrow> deriveRule (env N) r"
  "r \<in> ABS_RecvInvAck1s N \<Longrightarrow> deriveRule (env N) r"
  "r \<in> ABS_RecvReqEs N \<Longrightarrow> deriveRule (env N) r"
  "r \<in> ABS_RecvReqSs N \<Longrightarrow> deriveRule (env N) r"
  unfolding deriveRule_def deriveForm_def deriveStmt_def RecvGntEs_def RecvGntE_def RecvGntSs_def RecvGntS_def SendGntEs_def SendGntE_def SendGntSs_def SendGntS_def RecvInvAck1s_def RecvInvAck1_def RecvInvAck2s_def RecvInvAck2_def SendInvAcks_def SendInvAck_def SendInvs_def SendInv_def RecvReqEs_def RecvReqE_def RecvReqSs_def RecvReqS_def SendReqEs_def SendReqE_def SendReqSs_def SendReqS_def ABS_SendGntEs_def ABS_SendGntE_def ABS_SendGntSs_def ABS_SendGntS_def ABS_RecvInvAck1s_def ABS_RecvInvAck1_def ABS_RecvReqEs_def ABS_RecvReqE_def ABS_RecvReqSs_def ABS_RecvReqS_def
  apply auto
  done
...
lemma symProtAll: 
  ...
  using symRecvGntE(1) RecvGntEs_def symParaRuleInfSymRuleSet symParaRuleInfSymRuleSet2
  apply auto[1]
  using symRecvGntS(1) RecvGntSs_def symParaRuleInfSymRuleSet symParaRuleInfSymRuleSet2
  apply auto[1]
  using symSendGntE(1) SendGntEs_def symParaRuleInfSymRuleSet symParaRuleInfSymRuleSet2
  apply auto[1]
  using symSendGntS(1) SendGntSs_def symParaRuleInfSymRuleSet symParaRuleInfSymRuleSet2
  apply auto[1]
  using symRecvInvAck1(1) RecvInvAck1s_def symParaRuleInfSymRuleSet symParaRuleInfSymRuleSet2
  apply auto[1]
  using symRecvInvAck2(1) RecvInvAck2s_def symParaRuleInfSymRuleSet symParaRuleInfSymRuleSet2
  apply auto[1]
  using symSendInvAck(1) SendInvAcks_def symParaRuleInfSymRuleSet symParaRuleInfSymRuleSet2
  apply auto[1]
  using symSendInv(1) SendInvs_def symParaRuleInfSymRuleSet symParaRuleInfSymRuleSet2
  apply auto[1]
  using symRecvReqE(1) RecvReqEs_def symParaRuleInfSymRuleSet symParaRuleInfSymRuleSet2
  apply auto[1]
  using symRecvReqS(1) RecvReqSs_def symParaRuleInfSymRuleSet symParaRuleInfSymRuleSet2
  apply auto[1]
  using symSendReqE(1) SendReqEs_def symParaRuleInfSymRuleSet symParaRuleInfSymRuleSet2
  apply auto[1]
  using symSendReqS(1) SendReqSs_def symParaRuleInfSymRuleSet symParaRuleInfSymRuleSet2
  apply auto[1]
  done

lemma symCntrlProp: 
  "symParamForm2 N (CntrlProp N)"
  unfolding CntrlProp_def
  apply auto
...
lemma symRecvGntE_ref: 
  ...
  "i \<le> N \<Longrightarrow> wellFormedRule (env N) N (RecvGntE_ref i)"
  unfolding RecvGntE_ref_def
  apply (auto intro!: symParamRuleI2 symParamRuleI symParamFormAnd symParamFormForall symParamFormForallExcl symParamFormImply symParamStatementParallel symParamStatementForall symParamStatementForallExcl symParamStatementIte)
  unfolding symParamForm_def symParamStatement_def symParamForm2_def symParamStatement2_def mutualDiffVars_def equivForm_def
  apply auto
  done
...
lemma RecvGntE_strengthen: 
  ...
  unfolding lemmasFor_RecvGntE_def  RecvGntE_def RecvGntE_ref_def
  apply auto
  done

lemma abs_RecvGntE_ref: 
  "M \<le> N \<Longrightarrow> i \<le> M \<Longrightarrow> absTransfRule (env N) M (RecvGntE_ref i) = RecvGntE_ref i"
  "M \<le> N \<Longrightarrow> i > M \<Longrightarrow> absTransfRule (env N) M (RecvGntE_ref i) = skipRule"
  unfolding RecvGntE_ref_def RecvGntE_ref_def skipRule_def
...
lemma symRecvGntS_ref: 
  ...
  "i \<le> N \<Longrightarrow> wellFormedRule (env N) N (RecvGntS_ref i)"
  unfolding RecvGntS_ref_def
  apply (auto intro!: symParamRuleI2 symParamRuleI symParamFormAnd symParamFormForall symParamFormForallExcl symParamFormImply symParamStatementParallel symParamStatementForall symParamStatementForallExcl symParamStatementIte)
  unfolding symParamForm_def symParamStatement_def symParamForm2_def symParamStatement2_def mutualDiffVars_def equivForm_def
  apply auto
  done
...
lemma RecvGntS_strengthen: 
  ...
  unfolding lemmasFor_RecvGntS_def  RecvGntS_def RecvGntS_ref_def
  apply auto
  done

lemma abs_RecvGntS_ref: 
  "M \<le> N \<Longrightarrow> i \<le> M \<Longrightarrow> absTransfRule (env N) M (RecvGntS_ref i) = RecvGntS_ref i"
  "M \<le> N \<Longrightarrow> i > M \<Longrightarrow> absTransfRule (env N) M (RecvGntS_ref i) = skipRule"
  unfolding RecvGntS_ref_def RecvGntS_ref_def skipRule_def
  apply (auto simp add: Let_def)
  done
...
lemma symSendGntE_ref: 
  ...
  "i \<le> N \<Longrightarrow> wellFormedRule (env N) N (SendGntE_ref N i)"
  unfolding SendGntE_ref_def
  apply (auto intro!: symParamRuleI2 symParamRuleI symParamFormAnd symParamFormForall symParamFormForallExcl symParamFormImply symParamStatementParallel symParamStatementForall symParamStatementForallExcl symParamStatementIte)
  unfolding symParamForm_def symParamStatement_def symParamForm2_def symParamStatement2_def mutualDiffVars_def equivForm_def
  apply auto
  done
...
lemma SendGntE_strengthen: 
  ...
  unfolding lemmasFor_SendGntE_def  SendGntE_def SendGntE_ref_def
  apply auto
  done

lemma abs_SendGntE_ref: 
  "M \<le> N \<Longrightarrow> i \<le> M \<Longrightarrow> absTransfRule (env N) M (SendGntE_ref N i) = SendGntE_ref M i"
  "M \<le> N \<Longrightarrow> i > M \<Longrightarrow> absTransfRule (env N) M (SendGntE_ref N i) = ABS_SendGntE M"
  unfolding SendGntE_ref_def SendGntE_ref_def ABS_SendGntE_def
  apply (auto simp add: Let_def)
  done
...
lemma symSendGntS_ref: 
  ...
  "i \<le> N \<Longrightarrow> wellFormedRule (env N) N (SendGntS_ref i)"
  unfolding SendGntS_ref_def
  apply (auto intro!: symParamRuleI2 symParamRuleI symParamFormAnd symParamFormForall symParamFormForallExcl symParamFormImply symParamStatementParallel symParamStatementForall symParamStatementForallExcl symParamStatementIte)
  unfolding symParamForm_def symParamStatement_def symParamForm2_def symParamStatement2_def mutualDiffVars_def equivForm_def
  apply auto
  done
...
lemma SendGntS_strengthen: 
  ...
  unfolding lemmasFor_SendGntS_def  SendGntS_def SendGntS_ref_def
  apply auto
  done

lemma abs_SendGntS_ref: 
  "M \<le> N \<Longrightarrow> i \<le> M \<Longrightarrow> absTransfRule (env N) M (SendGntS_ref i) = SendGntS_ref i"
  "M \<le> N \<Longrightarrow> i > M \<Longrightarrow> absTransfRule (env N) M (SendGntS_ref i) = ABS_SendGntS M"
  unfolding SendGntS_ref_def SendGntS_ref_def ABS_SendGntS_def
  apply (auto simp add: Let_def)
  done
...
lemma symRecvInvAck1_ref: 
  ...
  "i \<le> N \<Longrightarrow> wellFormedRule (env N) N (RecvInvAck1_ref N i)"
  unfolding RecvInvAck1_ref_def
  apply (auto intro!: symParamRuleI2 symParamRuleI symParamFormAnd symParamFormForall symParamFormForallExcl symParamFormImply symParamStatementParallel symParamStatementForall symParamStatementForallExcl symParamStatementIte)
  unfolding symParamForm_def symParamStatement_def symParamForm2_def symParamStatement2_def mutualDiffVars_def equivForm_def
  apply auto
  done
...
lemma RecvInvAck1_strengthen: 
  ...
  unfolding lemmasFor_RecvInvAck1_def Lemma_1_def RecvInvAck1_def RecvInvAck1_ref_def
  apply auto
  done

lemma abs_RecvInvAck1_ref: 
  "M \<le> N \<Longrightarrow> i \<le> M \<Longrightarrow> absTransfRule (env N) M (RecvInvAck1_ref N i) = RecvInvAck1_ref M i"
  "M \<le> N \<Longrightarrow> i > M \<Longrightarrow> absTransfRule (env N) M (RecvInvAck1_ref N i) = ABS_RecvInvAck1 M"
  unfolding RecvInvAck1_ref_def RecvInvAck1_ref_def ABS_RecvInvAck1_def
  apply (auto simp add: Let_def)
  done
...
lemma symRecvInvAck2_ref: 
  ...
  "i \<le> N \<Longrightarrow> wellFormedRule (env N) N (RecvInvAck2_ref i)"
  unfolding RecvInvAck2_ref_def
  apply (auto intro!: symParamRuleI2 symParamRuleI symParamFormAnd symParamFormForall symParamFormForallExcl symParamFormImply symParamStatementParallel symParamStatementForall symParamStatementForallExcl symParamStatementIte)
  unfolding symParamForm_def symParamStatement_def symParamForm2_def symParamStatement2_def mutualDiffVars_def equivForm_def
  apply auto
  done
...
lemma RecvInvAck2_strengthen: 
  ...
  unfolding lemmasFor_RecvInvAck2_def  RecvInvAck2_def RecvInvAck2_ref_def
  apply auto
  done

lemma abs_RecvInvAck2_ref: 
  "M \<le> N \<Longrightarrow> i \<le> M \<Longrightarrow> absTransfRule (env N) M (RecvInvAck2_ref i) = RecvInvAck2_ref i"
  "M \<le> N \<Longrightarrow> i > M \<Longrightarrow> absTransfRule (env N) M (RecvInvAck2_ref i) = skipRule"
  unfolding RecvInvAck2_ref_def RecvInvAck2_ref_def skipRule_def
  apply (auto simp add: Let_def)
  done
...
lemma symSendInvAck_ref: 
  ...
  "i \<le> N \<Longrightarrow> wellFormedRule (env N) N (SendInvAck_ref i)"
  unfolding SendInvAck_ref_def
  apply (auto intro!: symParamRuleI2 symParamRuleI symParamFormAnd symParamFormForall symParamFormForallExcl symParamFormImply symParamStatementParallel symParamStatementForall symParamStatementForallExcl symParamStatementIte)
  unfolding symParamForm_def symParamStatement_def symParamForm2_def symParamStatement2_def mutualDiffVars_def equivForm_def
  apply auto
  done
...
lemma SendInvAck_strengthen: 
  ...
  unfolding lemmasFor_SendInvAck_def  SendInvAck_def SendInvAck_ref_def
  apply auto
  done

lemma abs_SendInvAck_ref: 
  "M \<le> N \<Longrightarrow> i \<le> M \<Longrightarrow> absTransfRule (env N) M (SendInvAck_ref i) = SendInvAck_ref i"
  "M \<le> N \<Longrightarrow> i > M \<Longrightarrow> absTransfRule (env N) M (SendInvAck_ref i) = skipRule"
  unfolding SendInvAck_ref_def SendInvAck_ref_def skipRule_def
  apply (auto simp add: Let_def)
  done
...
lemma symSendInv_ref: 
  ...
  "i \<le> N \<Longrightarrow> wellFormedRule (env N) N (SendInv_ref i)"
  unfolding SendInv_ref_def
  apply (auto intro!: symParamRuleI2 symParamRuleI symParamFormAnd symParamFormForall symParamFormForallExcl symParamFormImply symParamStatementParallel symParamStatementForall symParamStatementForallExcl symParamStatementIte)
  unfolding symParamForm_def symParamStatement_def symParamForm2_def symParamStatement2_def mutualDiffVars_def equivForm_def
  apply auto
  done
...
lemma SendInv_strengthen: 
  ...
  unfolding lemmasFor_SendInv_def  SendInv_def SendInv_ref_def
  apply auto
  done

lemma abs_SendInv_ref: 
  "M \<le> N \<Longrightarrow> i \<le> M \<Longrightarrow> absTransfRule (env N) M (SendInv_ref i) = SendInv_ref i"
  "M \<le> N \<Longrightarrow> i > M \<Longrightarrow> absTransfRule (env N) M (SendInv_ref i) = skipRule"
  unfolding SendInv_ref_def SendInv_ref_def skipRule_def
  apply (auto simp add: Let_def)
  done
...
lemma symRecvReqE_ref: 
  ...
  "i \<le> N \<Longrightarrow> wellFormedRule (env N) N (RecvReqE_ref N i)"
  unfolding RecvReqE_ref_def
  apply (auto intro!: symParamRuleI2 symParamRuleI symParamFormAnd symParamFormForall symParamFormForallExcl symParamFormImply symParamStatementParallel symParamStatementForall symParamStatementForallExcl symParamStatementIte)
  unfolding symParamForm_def symParamStatement_def symParamForm2_def symParamStatement2_def mutualDiffVars_def equivForm_def
  apply auto
  done
...
lemma RecvReqE_strengthen: 
  ...
  unfolding lemmasFor_RecvReqE_def  RecvReqE_def RecvReqE_ref_def
  apply auto
  done

lemma abs_RecvReqE_ref: 
  "M \<le> N \<Longrightarrow> i \<le> M \<Longrightarrow> absTransfRule (env N) M (RecvReqE_ref N i) = RecvReqE_ref M i"
  "M \<le> N \<Longrightarrow> i > M \<Longrightarrow> absTransfRule (env N) M (RecvReqE_ref N i) = ABS_RecvReqE M"
  unfolding RecvReqE_ref_def RecvReqE_ref_def ABS_RecvReqE_def
  apply (auto simp add: Let_def)
  done
...
lemma symRecvReqS_ref: 
  ...
  "i \<le> N \<Longrightarrow> wellFormedRule (env N) N (RecvReqS_ref N i)"
  unfolding RecvReqS_ref_def
  apply (auto intro!: symParamRuleI2 symParamRuleI symParamFormAnd symParamFormForall symParamFormForallExcl symParamFormImply symParamStatementParallel symParamStatementForall symParamStatementForallExcl symParamStatementIte)
  unfolding symParamForm_def symParamStatement_def symParamForm2_def symParamStatement2_def mutualDiffVars_def equivForm_def
  apply auto
  done
...
lemma RecvReqS_strengthen: 
  ...
  unfolding lemmasFor_RecvReqS_def  RecvReqS_def RecvReqS_ref_def
  apply auto
  done

lemma abs_RecvReqS_ref: 
  "M \<le> N \<Longrightarrow> i \<le> M \<Longrightarrow> absTransfRule (env N) M (RecvReqS_ref N i) = RecvReqS_ref M i"
  "M \<le> N \<Longrightarrow> i > M \<Longrightarrow> absTransfRule (env N) M (RecvReqS_ref N i) = ABS_RecvReqS M"
  unfolding RecvReqS_ref_def RecvReqS_ref_def ABS_RecvReqS_def
  apply (auto simp add: Let_def)
  done
...
lemma symSendReqE_ref: 
  ...
  "i \<le> N \<Longrightarrow> wellFormedRule (env N) N (SendReqE_ref i)"
  unfolding SendReqE_ref_def
  apply (auto intro!: symParamRuleI2 symParamRuleI symParamFormAnd symParamFormForall symParamFormForallExcl symParamFormImply symParamStatementParallel symParamStatementForall symParamStatementForallExcl symParamStatementIte)
  unfolding symParamForm_def symParamStatement_def symParamForm2_def symParamStatement2_def mutualDiffVars_def equivForm_def
  apply auto
  done
...
lemma SendReqE_strengthen: 
  ...
  unfolding lemmasFor_SendReqE_def  SendReqE_def SendReqE_ref_def
  apply auto
  done

lemma abs_SendReqE_ref: 
  "M \<le> N \<Longrightarrow> i \<le> M \<Longrightarrow> absTransfRule (env N) M (SendReqE_ref i) = SendReqE_ref i"
  "M \<le> N \<Longrightarrow> i > M \<Longrightarrow> absTransfRule (env N) M (SendReqE_ref i) = skipRule"
  unfolding SendReqE_ref_def SendReqE_ref_def skipRule_def
  apply (auto simp add: Let_def)
  done
...
lemma symSendReqS_ref: 
  ...
  "i \<le> N \<Longrightarrow> wellFormedRule (env N) N (SendReqS_ref i)"
  unfolding SendReqS_ref_def
  apply (auto intro!: symParamRuleI2 symParamRuleI symParamFormAnd symParamFormForall symParamFormForallExcl symParamFormImply symParamStatementParallel symParamStatementForall symParamStatementForallExcl symParamStatementIte)
  unfolding symParamForm_def symParamStatement_def symParamForm2_def symParamStatement2_def mutualDiffVars_def equivForm_def
  apply auto
  done
...
lemma SendReqS_strengthen: 
  ...
  unfolding lemmasFor_SendReqS_def  SendReqS_def SendReqS_ref_def
  apply auto
  done

lemma abs_SendReqS_ref: 
  "M \<le> N \<Longrightarrow> i \<le> M \<Longrightarrow> absTransfRule (env N) M (SendReqS_ref i) = SendReqS_ref i"
  "M \<le> N \<Longrightarrow> i > M \<Longrightarrow> absTransfRule (env N) M (SendReqS_ref i) = skipRule"
  unfolding SendReqS_ref_def SendReqS_ref_def skipRule_def
  apply (auto simp add: Let_def)
  done
...
lemma RecvGntEStrengthRel: 
  ...
  unfolding RecvGntEs_def RecvGntE_refs_def
  apply (rule_tac ?lemmasFor_r="lemmasFor_RecvGntE" in strengthenExt1)
  using RecvGntE_strengthen
  apply presburger
  unfolding InvS_def
  apply auto
  done
...
lemma RecvGntSStrengthRel: 
  ...
  unfolding RecvGntSs_def RecvGntS_refs_def
  apply (rule_tac ?lemmasFor_r="lemmasFor_RecvGntS" in strengthenExt1)
  using RecvGntS_strengthen
  apply presburger
  unfolding InvS_def
  apply auto
  done
...
lemma SendGntEStrengthRel: 
  ...
  unfolding SendGntEs_def SendGntE_refs_def
  apply (rule_tac ?lemmasFor_r="lemmasFor_SendGntE" in strengthenExt1)
  using SendGntE_strengthen
  apply presburger
  unfolding InvS_def
  apply auto
  done
...
lemma SendGntSStrengthRel: 
  ...
  unfolding SendGntSs_def SendGntS_refs_def
  apply (rule_tac ?lemmasFor_r="lemmasFor_SendGntS" in strengthenExt1)
  using SendGntS_strengthen
  apply presburger
  unfolding InvS_def
  apply auto
  done
...
lemma RecvInvAck1StrengthRel: 
  ...
  unfolding RecvInvAck1s_def RecvInvAck1_refs_def
  apply (rule_tac ?lemmasFor_r="lemmasFor_RecvInvAck1" in strengthenExt1)
  using RecvInvAck1_strengthen
  apply presburger
  unfolding InvS_def
  apply auto
  done
...
lemma RecvInvAck2StrengthRel: 
  ...
  unfolding RecvInvAck2s_def RecvInvAck2_refs_def
  apply (rule_tac ?lemmasFor_r="lemmasFor_RecvInvAck2" in strengthenExt1)
  using RecvInvAck2_strengthen
  apply presburger
  unfolding InvS_def
  apply auto
  done
...
lemma SendInvAckStrengthRel: 
  ...
  unfolding SendInvAcks_def SendInvAck_refs_def
  apply (rule_tac ?lemmasFor_r="lemmasFor_SendInvAck" in strengthenExt1)
  using SendInvAck_strengthen
  apply presburger
  unfolding InvS_def
  apply auto
  done
...
lemma SendInvStrengthRel: 
  ...
  unfolding SendInvs_def SendInv_refs_def
  apply (rule_tac ?lemmasFor_r="lemmasFor_SendInv" in strengthenExt1)
  using SendInv_strengthen
  apply presburger
  unfolding InvS_def
  apply auto
  done
...
lemma RecvReqEStrengthRel: 
  ...
  unfolding RecvReqEs_def RecvReqE_refs_def
  apply (rule_tac ?lemmasFor_r="lemmasFor_RecvReqE" in strengthenExt1)
  using RecvReqE_strengthen
  apply presburger
  unfolding InvS_def
  apply auto
  done
...
lemma RecvReqSStrengthRel: 
  ...
  unfolding RecvReqSs_def RecvReqS_refs_def
  apply (rule_tac ?lemmasFor_r="lemmasFor_RecvReqS" in strengthenExt1)
  using RecvReqS_strengthen
  apply presburger
  unfolding InvS_def
  apply auto
  done
...
lemma SendReqEStrengthRel: 
  ...
  unfolding SendReqEs_def SendReqE_refs_def
  apply (rule_tac ?lemmasFor_r="lemmasFor_SendReqE" in strengthenExt1)
  using SendReqE_strengthen
  apply presburger
  unfolding InvS_def
  apply auto
  done
...
lemma SendReqSStrengthRel: 
  ...
  unfolding SendReqSs_def SendReqS_refs_def
  apply (rule_tac ?lemmasFor_r="lemmasFor_SendReqS" in strengthenExt1)
  using SendReqS_strengthen
  apply presburger
  unfolding InvS_def
  apply auto
  done
...
lemma deriveAllRef: 
  "r \<in> RecvGntE_refs N \<Longrightarrow> deriveRule (env N) r"
  "r \<in> RecvGntS_refs N \<Longrightarrow> deriveRule (env N) r"
  "r \<in> SendGntE_refs N \<Longrightarrow> deriveRule (env N) r"
  "r \<in> SendGntS_refs N \<Longrightarrow> deriveRule (env N) r"
  "r \<in> RecvInvAck1_refs N \<Longrightarrow> deriveRule (env N) r"
  "r \<in> RecvInvAck2_refs N \<Longrightarrow> deriveRule (env N) r"
  "r \<in> SendInvAck_refs N \<Longrightarrow> deriveRule (env N) r"
  "r \<in> SendInv_refs N \<Longrightarrow> deriveRule (env N) r"
  "r \<in> RecvReqE_refs N \<Longrightarrow> deriveRule (env N) r"
  "r \<in> RecvReqS_refs N \<Longrightarrow> deriveRule (env N) r"
  "r \<in> SendReqE_refs N \<Longrightarrow> deriveRule (env N) r"
  "r \<in> SendReqS_refs N \<Longrightarrow> deriveRule (env N) r"
...
lemma Abs_RecvGntE_refs: 
  "M < N \<Longrightarrow> (absTransfRule (env N) M ` RecvGntE_refs N) = (RecvGntE_refs M \<union> {skipRule})"
  unfolding RecvGntE_refs_def
  apply (rule absGen)
  using abs_RecvGntE_ref
  apply auto
  done

lemma Abs_RecvGntS_refs: 
  "M < N \<Longrightarrow> (absTransfRule (env N) M ` RecvGntS_refs N) = (RecvGntS_refs M \<union> {skipRule})"
  unfolding RecvGntS_refs_def
  apply (rule absGen)
  using abs_RecvGntS_ref
  apply auto
  done

lemma Abs_SendGntE_refs: 
  "M < N \<Longrightarrow> (absTransfRule (env N) M ` SendGntE_refs N) = (SendGntE_refs M \<union> ABS_SendGntEs M)"
  unfolding SendGntE_refs_def ABS_SendGntEs_def
  apply (rule absGen)
  using abs_SendGntE_ref
  apply auto
  done

lemma Abs_SendGntS_refs: 
  "M < N \<Longrightarrow> (absTransfRule (env N) M ` SendGntS_refs N) = (SendGntS_refs M \<union> ABS_SendGntSs M)"
  unfolding SendGntS_refs_def ABS_SendGntSs_def
  apply (rule absGen)
  using abs_SendGntS_ref
  apply auto
  done

lemma Abs_RecvInvAck1_refs: 
  "M < N \<Longrightarrow> (absTransfRule (env N) M ` RecvInvAck1_refs N) = (RecvInvAck1_refs M \<union> ABS_RecvInvAck1s M)"
  unfolding RecvInvAck1_refs_def ABS_RecvInvAck1s_def
  apply (rule absGen)
  using abs_RecvInvAck1_ref
  apply auto
  done

lemma Abs_RecvInvAck2_refs: 
  "M < N \<Longrightarrow> (absTransfRule (env N) M ` RecvInvAck2_refs N) = (RecvInvAck2_refs M \<union> {skipRule})"
  unfolding RecvInvAck2_refs_def
  apply (rule absGen)
  using abs_RecvInvAck2_ref
  apply auto
  done

lemma Abs_SendInvAck_refs: 
  "M < N \<Longrightarrow> (absTransfRule (env N) M ` SendInvAck_refs N) = (SendInvAck_refs M \<union> {skipRule})"
  unfolding SendInvAck_refs_def
  apply (rule absGen)
  using abs_SendInvAck_ref
  apply auto
  done

lemma Abs_SendInv_refs: 
  "M < N \<Longrightarrow> (absTransfRule (env N) M ` SendInv_refs N) = (SendInv_refs M \<union> {skipRule})"
  unfolding SendInv_refs_def
  apply (rule absGen)
  using abs_SendInv_ref
  apply auto
  done

lemma Abs_RecvReqE_refs: 
  "M < N \<Longrightarrow> (absTransfRule (env N) M ` RecvReqE_refs N) = (RecvReqE_refs M \<union> ABS_RecvReqEs M)"
  unfolding RecvReqE_refs_def ABS_RecvReqEs_def
  apply (rule absGen)
  using abs_RecvReqE_ref
  apply auto
  done

lemma Abs_RecvReqS_refs: 
  "M < N \<Longrightarrow> (absTransfRule (env N) M ` RecvReqS_refs N) = (RecvReqS_refs M \<union> ABS_RecvReqSs M)"
  unfolding RecvReqS_refs_def ABS_RecvReqSs_def
  apply (rule absGen)
  using abs_RecvReqS_ref
  apply auto
  done

lemma Abs_SendReqE_refs: 
  "M < N \<Longrightarrow> (absTransfRule (env N) M ` SendReqE_refs N) = (SendReqE_refs M \<union> {skipRule})"
  unfolding SendReqE_refs_def
  apply (rule absGen)
  using abs_SendReqE_ref
  apply auto
  done

lemma Abs_SendReqS_refs: 
  "M < N \<Longrightarrow> (absTransfRule (env N) M ` SendReqS_refs N) = (SendReqS_refs M \<union> {skipRule})"
  unfolding SendReqS_refs_def
  apply (rule absGen)
  using abs_SendReqS_ref
  apply auto
  done
...
lemma strengthenVsObsLs_lemmasFor_RecvGntE: 
  ...
  unfolding strengthenVsObsLs_def lemmasFor_RecvGntE_def lemmasFor_RecvGntE'_def
  apply auto
  done

lemma lemmaRecvGntE_fitEnv: 
  "formEval (pre r) s \<Longrightarrow> fitEnv s (env N) \<Longrightarrow> r \<in> RecvGntE_refs N \<Longrightarrow> fitEnv (trans1 (act r) s) (env N)"
  unfolding RecvGntE_refs_def RecvGntE_ref_def
  apply auto
  done
...
lemma strengthenVsObsLs_lemmasFor_RecvGntS: 
  ...
  unfolding strengthenVsObsLs_def lemmasFor_RecvGntS_def lemmasFor_RecvGntS'_def
  apply auto
  done

lemma lemmaRecvGntS_fitEnv: 
  "formEval (pre r) s \<Longrightarrow> fitEnv s (env N) \<Longrightarrow> r \<in> RecvGntS_refs N \<Longrightarrow> fitEnv (trans1 (act r) s) (env N)"
  unfolding RecvGntS_refs_def RecvGntS_ref_def
  apply auto
  done
...
lemma strengthenVsObsLs_lemmasFor_SendGntE: 
  ...
  unfolding strengthenVsObsLs_def lemmasFor_SendGntE_def lemmasFor_SendGntE'_def
  apply auto
  done

lemma lemmaSendGntE_fitEnv: 
  "formEval (pre r) s \<Longrightarrow> fitEnv s (env N) \<Longrightarrow> r \<in> SendGntE_refs N \<Longrightarrow> fitEnv (trans1 (act r) s) (env N)"
  unfolding SendGntE_refs_def SendGntE_ref_def
  apply auto
  done
...
lemma strengthenVsObsLs_lemmasFor_SendGntS: 
  ...
  unfolding strengthenVsObsLs_def lemmasFor_SendGntS_def lemmasFor_SendGntS'_def
  apply auto
  done

lemma lemmaSendGntS_fitEnv: 
  "formEval (pre r) s \<Longrightarrow> fitEnv s (env N) \<Longrightarrow> r \<in> SendGntS_refs N \<Longrightarrow> fitEnv (trans1 (act r) s) (env N)"
  unfolding SendGntS_refs_def SendGntS_ref_def
  apply auto
  done
...
lemma strengthenVsObsLs_lemmasFor_RecvInvAck1: 
  ...
  unfolding strengthenVsObsLs_def lemmasFor_RecvInvAck1_def lemmasFor_RecvInvAck1'_def
  apply (auto intro: strengthenVsObsLemma_1)
  done

lemma lemmaRecvInvAck1_fitEnv: 
  "formEval (pre r) s \<Longrightarrow> fitEnv s (env N) \<Longrightarrow> r \<in> RecvInvAck1_refs N \<Longrightarrow> fitEnv (trans1 (act r) s) (env N)"
  unfolding RecvInvAck1_refs_def RecvInvAck1_ref_def
  apply auto
  done
...
lemma strengthenVsObsLs_lemmasFor_RecvInvAck2: 
  ...
  unfolding strengthenVsObsLs_def lemmasFor_RecvInvAck2_def lemmasFor_RecvInvAck2'_def
  apply auto
  done

lemma lemmaRecvInvAck2_fitEnv: 
  "formEval (pre r) s \<Longrightarrow> fitEnv s (env N) \<Longrightarrow> r \<in> RecvInvAck2_refs N \<Longrightarrow> fitEnv (trans1 (act r) s) (env N)"
  unfolding RecvInvAck2_refs_def RecvInvAck2_ref_def
  apply auto
  done
...
lemma strengthenVsObsLs_lemmasFor_SendInvAck: 
  ...
  unfolding strengthenVsObsLs_def lemmasFor_SendInvAck_def lemmasFor_SendInvAck'_def
  apply auto
  done

lemma lemmaSendInvAck_fitEnv: 
  "formEval (pre r) s \<Longrightarrow> fitEnv s (env N) \<Longrightarrow> r \<in> SendInvAck_refs N \<Longrightarrow> fitEnv (trans1 (act r) s) (env N)"
  unfolding SendInvAck_refs_def SendInvAck_ref_def
  apply auto
  done
...
lemma strengthenVsObsLs_lemmasFor_SendInv: 
  ...
  unfolding strengthenVsObsLs_def lemmasFor_SendInv_def lemmasFor_SendInv'_def
  apply auto
  done

lemma lemmaSendInv_fitEnv: 
  "formEval (pre r) s \<Longrightarrow> fitEnv s (env N) \<Longrightarrow> r \<in> SendInv_refs N \<Longrightarrow> fitEnv (trans1 (act r) s) (env N)"
  unfolding SendInv_refs_def SendInv_ref_def
  apply auto
  done
...
lemma strengthenVsObsLs_lemmasFor_RecvReqE: 
  ...
  unfolding strengthenVsObsLs_def lemmasFor_RecvReqE_def lemmasFor_RecvReqE'_def
  apply auto
  done

lemma lemmaRecvReqE_fitEnv: 
  "formEval (pre r) s \<Longrightarrow> fitEnv s (env N) \<Longrightarrow> r \<in> RecvReqE_refs N \<Longrightarrow> fitEnv (trans1 (act r) s) (env N)"
  unfolding RecvReqE_refs_def RecvReqE_ref_def
  apply auto
  done
...
lemma strengthenVsObsLs_lemmasFor_RecvReqS: 
  ...
  unfolding strengthenVsObsLs_def lemmasFor_RecvReqS_def lemmasFor_RecvReqS'_def
  apply auto
  done

lemma lemmaRecvReqS_fitEnv: 
  "formEval (pre r) s \<Longrightarrow> fitEnv s (env N) \<Longrightarrow> r \<in> RecvReqS_refs N \<Longrightarrow> fitEnv (trans1 (act r) s) (env N)"
  unfolding RecvReqS_refs_def RecvReqS_ref_def
  apply auto
  done
...
lemma strengthenVsObsLs_lemmasFor_SendReqE: 
  ...
  unfolding strengthenVsObsLs_def lemmasFor_SendReqE_def lemmasFor_SendReqE'_def
  apply auto
  done

lemma lemmaSendReqE_fitEnv: 
  "formEval (pre r) s \<Longrightarrow> fitEnv s (env N) \<Longrightarrow> r \<in> SendReqE_refs N \<Longrightarrow> fitEnv (trans1 (act r) s) (env N)"
  unfolding SendReqE_refs_def SendReqE_ref_def
  apply auto
  done
...
lemma strengthenVsObsLs_lemmasFor_SendReqS: 
  ...
  unfolding strengthenVsObsLs_def lemmasFor_SendReqS_def lemmasFor_SendReqS'_def
  apply auto
  done

lemma lemmaSendReqS_fitEnv: 
  "formEval (pre r) s \<Longrightarrow> fitEnv s (env N) \<Longrightarrow> r \<in> SendReqS_refs N \<Longrightarrow> fitEnv (trans1 (act r) s) (env N)"
  unfolding SendReqS_refs_def SendReqS_ref_def
  apply auto
  done
...
lemma ReachStafitEnv: 
  ...
  subgoal for r sk
    unfolding rule_refs_def
    apply (auto intro: Un_iff lemmaRecvGntE_fitEnv lemmaRecvGntS_fitEnv lemmaSendGntE_fitEnv lemmaSendGntS_fitEnv lemmaRecvInvAck1_fitEnv lemmaRecvInvAck2_fitEnv lemmaSendInvAck_fitEnv lemmaSendInv_fitEnv lemmaRecvReqE_fitEnv lemmaRecvReqS_fitEnv lemmaSendReqE_fitEnv lemmaSendReqS_fitEnv)
  done
  done
...
lemma absProtSim: 
  ...
  subgoal
    ...
    apply auto
    subgoal
      using strengthenVsObsLs_lemmasFor_RecvGntE
      apply auto
    done
    subgoal
      using strengthenVsObsLs_lemmasFor_RecvGntS
      apply auto
    done
    subgoal
      using strengthenVsObsLs_lemmasFor_SendGntE
      apply auto
    done
    subgoal
      using strengthenVsObsLs_lemmasFor_SendGntS
      apply auto
    done
    subgoal
      using strengthenVsObsLs_lemmasFor_RecvInvAck1
      apply auto
    done
    subgoal
      using strengthenVsObsLs_lemmasFor_RecvInvAck2
      apply auto
    done
    subgoal
      using strengthenVsObsLs_lemmasFor_SendInvAck
      apply auto
    done
    subgoal
      using strengthenVsObsLs_lemmasFor_SendInv
      apply auto
    done
    subgoal
      using strengthenVsObsLs_lemmasFor_RecvReqE
      apply auto
    done
    subgoal
      using strengthenVsObsLs_lemmasFor_RecvReqS
      apply auto
    done
    subgoal
      using strengthenVsObsLs_lemmasFor_SendReqE
      apply auto
    done
    subgoal
      using strengthenVsObsLs_lemmasFor_SendReqS
      apply auto
    done
  done
