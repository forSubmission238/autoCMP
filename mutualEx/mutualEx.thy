...
primrec env :: "nat => envType" where
  "env N (Ident n) = (if Ident n = Ident ''x'' then boolType else anyType)" |
...
lemma symProtAllRef: 
  ...
  using symTry_ref(1) Try_refs_def symCrit_ref(1) Crit_refs_def symExit_ref(1) Exit_refs_def symIdle_ref(1) Idle_refs_def symParaRuleInfSymRuleSet symParaRuleInfSymRuleSet2
  apply auto
  done
